...
- [구현](#구현)
  - [구현 내용](#구현-내용)
    - [1. 토론 주제 로딩 무한 스크롤](#1-토론-주제-로딩-무한-스크롤)
    - [2. 디바운스를 활용하여 토론 생성 전 기존 토론 주제 검색](#2-디바운스를-활용하여-토론-생성-전-기존-토론-주제-검색)
    - [3. 의견 리스트 반응형 레이아웃](#3-의견-리스트-반응형-레이아웃)
    - [4. 의견 관계를 그래프로 시각화](#4-의견-관계를-그래프로-시각화)
    - [5. 모달로 생성한 로그인 및 회원가입](#5-모달로-생성한-로그인-및-회원가입)
    - [6. 로그인한 유저 정보 관리](#6-로그인한-유저-정보-관리)
  - [구현중 해결한 문제](#구현중-해결한-문제)
      - [문제 1. 고정된 헤더와 어사이드를 유지하며 메인에 스크롤 적용](#문제-1-고정된-헤더와-어사이드를-유지하며-메인에-스크롤-적용)
      - [문제 2. console.log 코드 관리](#문제-2-consolelog-코드-관리)
      - [문제 3. 인풋 엘리먼트가 중복되어 재사용 가능한 컴포넌트로 만듦](#문제-3-인풋-엘리먼트가-중복되어-재사용-가능한-컴포넌트로-만듦)
      - [문제 4. 의견 작성, 수정, 덧붙임 컴포넌트가 비대해짐](#문제-4-의견-작성-수정-덧붙임-컴포넌트가-비대해짐)
      - [문제 5. 인증 폼이 복잡하고 중복되는 코드가 너무 많음](#문제-5-인증-폼이-복잡하고-중복되는-코드가-너무-많음)
      - [문제 6. 서버로부터 받은 데이터를 자동으로 변환](#문제-6-서버로부터-받은-데이터를-자동으로-변환)
...
- Vue.js: Vue.js를 선택한 이유는 싱글 파일 컴포넌트(SFC)라는 특징 때문이다. 기존의 JS, CSS, HTML을 분리하여 다루는 관점과는 다르게 하나의 컴포넌트로 관리할 수 있다는 점이 유지보수에 유리하다고 생각했다.
- TypeScript: JS는 사용하기 편하지만, 유지보수에 불편한 점이 있다. 코드를 읽는 것만으로는 변수에 어떤 데이터가 들어있는지 알기 어렵기 때문이다. TS는 타입 정보를 통해 코드를 읽을 때 더 많은 정보를 얻을 수 있다.
- Pinia: Pinia는 상태관리 라이브러리이다. Vue.js 3버전의 기본 상태 관리 라이브러리로 지정되어 문서가 잘 되어 있어 선택하였다.
- AWS Lightsail: Lightsail을 선택한 이유는 웹 개발에 필요한 서비스를 한데 모아 제공하는 편리함과 상당한 수준의 기본 트래픽을 제공해 주었기 때문이다.
- Docker: Github Action과 Docker를 기반으로 배포 자동화를 구축했다. 개발자가 일일이 신경 쓰는 것은 실수를 유발하기 쉽기 때문이다. Docker는 개발 환경 구축이 쉽고 그대로 배포에도 활용할 수 있어서 선택했다.
...
![그래프](./references/그래프.gif)

### 5. 모달로 생성한 로그인 및 회원가입
...
### 문제 3. 인풋 엘리먼트가 중복되어 재사용 가능한 컴포넌트로 만듦
